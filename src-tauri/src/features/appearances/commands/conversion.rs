...
fn complete_frame_group_to_proto(group: &CompleteFrameGroup) -> FrameGroup {
    ...
    frame_group.sprite_info = group.sprite_info.as_ref().map(complete_sprite_info_to_proto);
    frame_group
}
...
pub fn complete_flags_to_proto(flags: &CompleteFlags) -> AppearanceFlags {
    ...
    proto.bank = flags.bank.as_ref().map(
        |FlagBank {
             waypoints,
         }| AppearanceFlagBank {
            waypoints: *waypoints,
    ...
    proto.write = flags.write.as_ref().map(
        |FlagWrite {
             max_text_length,
         }| AppearanceFlagWrite {
    ...
    proto.hook = flags.hook.as_ref().map(
        |FlagHook {
             direction,
         }| AppearanceFlagHook {
            direction: *direction,
    ...
    proto.light = flags.light.as_ref().map(
        |FlagLight {
             brightness,
             color,
         }| AppearanceFlagLight {
    ...
    proto.shift = flags.shift.as_ref().map(
        |FlagShift {
             x,
             y,
         }| AppearanceFlagShift {
    ...
    proto.height = flags.height.as_ref().map(
        |FlagHeight {
             elevation,
    ...
    proto.automap = flags.automap.as_ref().map(
        |FlagAutomap {
             color,
    ...
    proto.lenshelp = flags.lenshelp.as_ref().map(
        |FlagLenshelp {
             id,
    ...
    proto.clothes = flags.clothes.as_ref().map(
        |FlagClothes {
             slot,
    ...
    proto.default_action = flags.default_action.as_ref().map(
        |FlagDefaultAction {
             action,
    ...
    proto.cyclopediaitem = flags.cyclopedia_item.as_ref().map(
        |FlagCyclopedia {
             cyclopedia_type,
    ...
    proto.imbueable = flags.imbueable.as_ref().map(
        |FlagImbueable {
             slot_count,
    ...
    proto.proficiency = flags.proficiency.as_ref().map(
        |FlagProficiency {
             proficiency_id,
