...
{#if spriteInfo}
  <div class="texture-form-section texture-bounding-section">
    ...
    <div class="texture-bounding-boxes">
        ...
        <tbody id="texture-bounding-box-body">
          ...
          {:else}
            {#each spriteInfo.bounding_boxes as box, i}
              <tr>
                <td>{i + 1}</td>
                <td><input type="number" class="texture-box-x" min="0" bind:value={box.x} on:change /></td>
                <td><input type="number" class="texture-box-y" min="0" bind:value={box.y} on:change /></td>
                <td><input type="number" class="texture-box-width" min="0" bind:value={box.width} on:change /></td>
                <td><input type="number" class="texture-box-height" min="0" bind:value={box.height} on:change /></td>
                <td>
                  <button type="button" class="texture-remove-box" on:click={() => removeBoundingBox(i)}>&times;</button>
                </td>
              </tr>
            {/each}
          {/if}
      ...
      <button id="texture-add-bounding-box" class="btn-secondary" on:click={addBoundingBox}>{translate('texture.bounding.button.add')}</button>
    </div>
  </div>
{/if}
