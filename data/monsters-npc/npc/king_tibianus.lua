...
npcConfig.voices = {
	interval = 15000,
	chance = 50,
	{ text = "Se voce nao seguir as regras dessa cidade, voce vai ser banido." },
	{ text = "Eu estou tao cansado de ter que ficar revisando esses documentos... Ser rei e tao dificil." },
	{ text = "Quando voce vai parar de ser cone e ser promovido? Voce tem que ser mais inteligente." },
	{ text = "O que eu faco nao interessa, nem nada da minha vida, vai pra la rale." },
	{ text = "Por acaso voce disse BAN?" },
	{ text = "Sou rei desde quando essa cidade foi fundada, 5.0, 6.4.. 7.4.. Ah sei la, to velho ja." },
}
		...
		elseif npcHandler:getTopic(playerId) == 3 then -- ARMOR/OUTFIT
			if player:getStorageValue(Storage.Quest.U12_15.GoldenOutfits) < 1 then
				if player:getMoney() + player:getBankBalance() >= 500000000 then
					...
					local inboxItems = inbox:getItemsCount()
