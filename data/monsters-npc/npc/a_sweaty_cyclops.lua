...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif MsgContains(message, "yes") and npcHandler:getTopic(playerId) == 9 then
		if player:getItemCount(5944) > 0 then
			...
			npcHandler:say("Lil' one no have soul orbs. Me no can make bolts.", npc, creature)
			npcHandler:setTopic(playerId, 9)
		end
	end

	local playerMoney = player:getMoney() + player:getBankBalance()
	if MsgContains(message, "amulet") then
		npcHandler:say("Me can make unbroken amulet. Lil' one want {cheap} way (5000 gold coins and pieces) or {expensive} way (50.000 gols coins, no pieces)?", npc, creature)
		npcHandler:setTopic(playerId, 10)
	elseif MsgContains(message, "cheap") and npcHandler:getTopic(playerId) == 10 then
		if playerMoney >= 5000 and player:getItemCount(7528) >= 1 and player:getItemCount(7529) >= 1 and player:getItemCount(7530) >= 1 and player:getItemCount(7531) >= 1 then
			...
			npcHandler:say("Big Ben makes lil' amulet unbroken! Here it is!", npc, creature)
			npcHandler:setTopic(playerId, 0)
		else
			npcHandler:say("Lil' one missing pieces or gold. Need 5000 gold coins and four broken pieces.", npc, creature)
			npcHandler:setTopic(playerId, 10)
		end
	elseif MsgContains(message, "expensive") and npcHandler:getTopic(playerId) == 10 then
		if playerMoney >= 50000 then
			player:removeMoneyBank(50000)
			player:addItem(7532, 1)
			npcHandler:say("Rich lil' one! Here your shiny amulet is!", npc, creature)
			npcHandler:setTopic(playerId, 0)
		else
			npcHandler:say("Lil' one no have enough gold. Need 50.000 gold coins.", npc, creature)
			npcHandler:setTopic(playerId, 10)
		end
	end
