...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "shortcut") and player:getStorageValue(Storage.Quest.U13_40.Podzilla.Access.Shortcut) == 1 then
	...
	elseif MsgContains(message, "yes") and npcHandler:getTopic(playerId) == 3 then
		...
		player:teleportTo(Position(33849, 32011, 6))
	elseif MsgContains(message, "gunther") and player:getStorageValue(Storage.Quest.U13_40.Podzilla.Access.Gunther) == 1 then
		npcHandler:say("Ah, yes. I am aware you are to rendezvous with a ship at the high seas. Are you ready to embark? {yes} or no?", npc, creature)
		npcHandler:setTopic(playerId, 1)
	elseif MsgContains(message, "yes") and npcHandler:getTopic(playerId) == 1 then
		if player:kv():get("podzilla-boat-fail-exhaust") and os.time() < player:kv():get("podzilla-boat-fail-exhaust") then
			npcHandler:say("Your boat is still being repaired from the damages caused by the sea!", npc, creature)
			npcHandler:setTopic(playerId, 0)
			return true
		end
		npcHandler:say("Set the sails!", npc, creature)
		npcHandler:setTopic(playerId, 0)
		player:teleportTo(Position(33972, 31976, 8))
	end
	return true
end
