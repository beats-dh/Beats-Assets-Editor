			...
			if not chargeItem[charge] then
				npcHandler:say("Sorry, you don't have an unenchanted " .. charge .. ".", npc, creature)
                               ...
                               if (player:getItemCount(npc:getCurrency()) >= chargePrice) and (player:getItemCount(chargeItem[charge].noChargeID) >= 1) then
                                       ...
                                       local itemAdd = Game.createItem(chargeItem[charge].ChargeID, 1)
                                       if player:addItemEx(itemAdd, false, INDEX_WHEREEVER) ~= RETURNVALUE_NOERROR then
                                               itemAdd:remove()
                                               npcHandler:say("Make some room in your inventory first.", npc, creature)
                                       else
