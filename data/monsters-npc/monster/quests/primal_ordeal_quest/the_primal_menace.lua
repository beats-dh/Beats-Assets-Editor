...
monster.loot = {	{ id = 60080, chance = 25000 }, -- boss token
}
...
local function spawnPods(monster, hazardPoints)
	...
	for i = 1, count do
		...
		if not Tile(spawnPosition):hasFlag(TILESTATE_PROTECTIONZONE) then
...
local function handlePrimalBeasts(monster)
	...
	for index, beastData in pairs(primalBeasts) do
		...
		elseif os.time() - created > 20 and primalMonster:getHealth() > 0 then
			...
			if position and not Tile(position):hasFlag(TILESTATE_PROTECTIONZONE) then
