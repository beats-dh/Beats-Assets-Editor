<script lang="ts">
  ...
  let spriteItems = $derived(visibleRange.map(spriteId => ({
    id: `sprite-${spriteId}`,
    spriteId,
  })));
  ...
  function handleSearch() { loadSprites(searchInput); }
  ...
  function goToNextPage() {
    if (order === 'asc') { pageStart = Math.max(1, pageStart + pageSize); }
    else { pageStart = Math.max(1, pageStart - pageSize); }
    loadPageFromState();
  }

  function goToPrevPage() {
    if (order === 'asc') { pageStart = Math.max(1, pageStart - pageSize); }
    else { pageStart = pageStart + pageSize; }
    loadPageFromState();
  }
  ...
  function handleSpriteClick(event: MouseEvent, spriteId: number) {
    ...
    if (!multiSelect) {
      ...
      if (selectedIds.has(spriteId)) { selectedIds.delete(spriteId); }
      else { selectedIds.add(spriteId); }
    }
  ...
  <div id="sprite-library-drawer" class="is-open" aria-hidden="false">
    ...
    <div class="sprite-library-panel" role="dialog" aria-label={translate('texture.library.title')}>
      <div class="sprite-library-header">
        ...
        <button type="button" class="close-btn" id="sprite-library-close" aria-label={translate('texture.library.button.close')} onclick={closeDrawer}>✕</button>
      </div>
      ...
      <div class="sprite-library-controls sprite-library-controls--grid">
        <label class="sprite-library-control">
          ...
          <input id="sprite-library-start" type="number" min="1" bind:value={pageStart} onchange={() => loadPageFromState()} />
        </label>
        <label class="sprite-library-control">
          <span>{translate('texture.library.pageSize')}</span>
          <input id="sprite-library-size" type="number" min="1" max="500" bind:value={pageSize} onchange={() => loadPageFromState()} />
        </label>
        <label class="sprite-library-control order-control">
          ...
          <div class="sprite-library-order-row">
            <button type="button" id="sprite-library-order" class="btn-secondary sprite-library-order-btn" data-order={order} onclick={toggleOrder}>
              {order === 'asc' ? translate('texture.library.order.asc') : translate('texture.library.order.desc')}
            </button>
            <div class="sprite-library-pager">
              <button type="button" id="sprite-library-prev" class="btn-secondary" title={translate('texture.library.prev')} onclick={goToPrevPage}>◀</button>
              <button type="button" id="sprite-library-next" class="btn-secondary" title={translate('texture.library.next')} onclick={goToNextPage}>▶</button>
            </div>
          </div>
        </label>
      </div>
      ...
      <div class="sprite-library-controls">
        <input id="sprite-library-search" type="text" bind:value={searchInput} placeholder={translate('texture.library.searchPlaceholder')} onkeydown={handleKeyDown} />
        <button type="button" id="sprite-library-search-btn" class="btn-secondary" onclick={handleSearch}>{translate('texture.library.button.search')}</button>
      </div>

      <p class="sprite-library-hint">{translate('texture.library.hint')}</p>

      <!-- svelte-ignore a11y_no_static_element_interactions -->
      <div class="sprite-library-list" id="sprite-library-list"
        use:dndzone={{ items: spriteItems, flipDurationMs: 200, dropTargetStyle: {}, type: 'sprite' }}
