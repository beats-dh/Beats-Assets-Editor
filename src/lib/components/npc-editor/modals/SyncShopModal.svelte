<script lang="ts">
  ...
  function parseGoldPrice(priceCell: Element): number | null {
    ...
    const hasCoinIndicator = Array.from(priceCell.querySelectorAll("img")).some(
      (img) => {
  ...
  function parseTradeRowsFromSection(
    renderedHtml: string,
    sectionId: "Buys" | "Sells",
  ): Array<{ itemName: string; price: number | null }> {
  ...
  async function fetchFandomCategoryTitles(): Promise<Map<string, string>> {
    ...
    while (true) {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch Fandom category list (${response.status})`,
        );
      }
  ...
  async function fetchFandomNpcRenderedHtml(
    pageTitle: string,
  ): Promise<string> {
    const params = new URLSearchParams({
      action: "parse",
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch Fandom page '${pageTitle}' (${response.status})`,
      );
    }
  ...
  async function buildFandomShopMap(
    npcNames: string[],
  ): Promise<FandomBuildResult> {
    ...
    const uniqueNpcNames = Array.from(
      new Set(
        npcNames.map((name) => name.trim()).filter((name) => name.length > 0),
      ),
    );
  ...
  async function handleSync() {
    ...
    try {
      ...
      if (shopSource === "fandom") {
        ...
        const npcNames = npcEntries
          .map((entry) => entry.name)
          .filter((name) => name.trim().length > 0);
        if (npcNames.length === 0) {
          throw new Error(
            "No NPC list loaded. Load NPC files first to map Fandom data.",
          );
        }
      ...
      if (shopSource === "fandom" && missingFandomNpcNames.length > 0) {
        ...
        const remaining =
          missingFandomNpcNames.length -
          Math.min(15, missingFandomNpcNames.length);
  ...
  >
    <div class="monster-modal" style="max-width: 650px;">
      <div class="monster-modal-header">
        <h3>{translate("npc.modal.sync.title")}</h3>
      ...
      <div class="monster-modal-body">
        <div class="monster-modal-section">
          <p style="color: var(--text-secondary); margin-bottom: 12px;">
            {translate("npc.modal.sync.desc")}
          </p>

          <div class="monster-modal-field" style="margin-bottom: 12px;">
            <label for="sync-shop-source"
              >{translate("npc.modal.sync.source")}</label
            >
            <select
              id="sync-shop-source"
              bind:value={shopSource}
              disabled={isSyncing}
            >
              <option value="proto"
                >{translate("npc.modal.sync.source.proto")}</option
              >
              <option value="fandom"
                >{translate("npc.modal.sync.source.fandom")}</option
              >
            </select>
            <small style="color: var(--text-secondary);">
              {#if shopSource === "proto"}Uses <code>npcsaledata</code> from the
                loaded appearances file.{:else}Uses <code
                  >https://tibia.fandom.com/wiki/Category:NPCs</code
                > and each NPC trade table.{/if}
            </small>
          </div>
          ...
          <div class="monster-modal-field" style="margin-bottom: 12px;">
            ...
            <small style="color: var(--text-secondary);">
              Default: <code>data/items/items.xml</code>. Accepts relative or
              absolute path.
            </small>
          </div>

          <div class="monster-modal-field" style="margin-bottom: 12px;">
            <label for="sync-ignore-item-ids"
              >{translate("npc.modal.sync.ignoreIds")}</label
            >
            <input
            ...
            />
            <small style="color: var(--text-secondary);"
              >Items with these client IDs will not be synced from the selected
              source.</small
            >
          </div>

          <div class="monster-modal-field" style="margin-bottom: 12px;">
            <label for="sync-ignore-item-names"
              >{translate("npc.modal.sync.ignoreNames")}</label
            >
            <input
            ...
            />
            <small style="color: var(--text-secondary);"
              >Items with these names will not be synced from the selected
              source.</small
            >
          </div>
          ...
          <div class="monster-modal-field" style="margin-bottom: 16px;">
            ...
            >
              <input
                type="checkbox"
                bind:checked={keepCustomItems}
                disabled={isSyncing}
              />
              {translate("npc.modal.sync.keepCustom")}
            </label>
            <small style="color: var(--text-secondary);">
              When enabled, existing shop items that are not found in the
              selected source are preserved. When disabled, NPC shops are
              rewritten using only the selected source.
            </small>
          </div>
        </div>
        ...
        {#if error}
          <div
            class="monster-modal-section"
            style="color: var(--danger-color); margin-top: 8px;"
          >
            {error}
          </div>
        {/if}
        ...
        {#if result}
          <div class="monster-modal-section" style="margin-top: 8px;">
            ...
            >
              <div>
                <strong>{translate("npc.modal.sync.result.scanned")}</strong>
                {result.totalNpcsScanned}
              </div>
              <div>
                <strong>{translate("npc.modal.sync.result.updated")}</strong>
                {result.npcsUpdated}
              </div>
              <div>
                <strong>{translate("npc.modal.sync.result.skipped")}</strong>
                {result.npcsSkipped}
              </div>
              <div>
                <strong>{translate("npc.modal.sync.result.added")}</strong>
                {result.itemsAdded}
              </div>
              <div>
                <strong>{translate("npc.modal.sync.result.removed")}</strong>
                {result.itemsRemoved}
              </div>
            </div>
            ...
            {#if result.errors.length > 0}
              <div style="margin-top: 8px;">
                <strong style="color: var(--danger-color);"
                  >Errors ({result.errors.length}):</strong
                >
                <div
                  class="modal-list"
                  style="max-height: 120px; overflow-y: auto; margin-top: 4px;"
                >
                  {#each result.errors as err}
                    <div
                      style="color: var(--danger-color); font-size: 0.85em; padding: 2px 0;"
                    >
                      {err}
                    </div>
                  {/each}
            ...
            {#if result.npcDetails.length > 0}
              <div style="margin-top: 8px;">
                ...
                <div
                  class="modal-list"
                  style="max-height: 150px; overflow-y: auto; margin-top: 4px;"
                >
                  {#each result.npcDetails as detail}
                    <div style="font-size: 0.85em; padding: 2px 0;">
                      <strong>{detail.npcName}</strong> - {detail.itemsBefore} items
                      -> {detail.itemsAfter} items
                    </div>
                  {/each}
                </div>
              </div>
      ...
      <div class="monster-modal-footer">
        <button class="btn-secondary" onclick={close}>
          {result
            ? translate("modal.btn.close")
            : translate("modal.btn.cancel")}
        </button>
        {#if !result}
          <button class="btn-primary" onclick={handleSync} disabled={isSyncing}>
            {isSyncing
              ? translate("npc.modal.sync.btn.syncing")
              : translate("npc.modal.sync.btn.syncAll")}
          </button>
        {/if}
      </div>
    </div>
