<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      if (!npcState.npcsRootPath) {
        try {
          const savedPath = await invoke<string | null>(COMMANDS.GET_NPC_BASE_PATH);
...
<div class="monster-editor-container">
  <header class="monster-editor-header">
    ...
    <div class="monster-header-actions">
      ...
      <button type="button" class="editor-icon-button" onclick={handleChangeDirectory} title="Escolher um novo diretório de NPCs">
        <span class="btn-icon">📁</span>
