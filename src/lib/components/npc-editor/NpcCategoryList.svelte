<script lang="ts">
  ...
  import { buildNpcTree, sortCategoryNodes, type NpcCategoryNode } from './utils';
  ...
  let sortedRootChildren = $derived(sortCategoryNodes(Array.from(tree.children.values())));
  let sortedRootNpcs = $derived([...tree.npcs].sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: 'base' })));
