<script lang="ts">
  ...
  interface Props { label: string; value: number; onChange: (val: number) => void; }
  ...
  function handleColorInput(e: Event) {
    ...
    if (targetIndex === -1) { const rgb = hexToRgb(normalized); targetIndex = findClosestColorId(rgb); }
...
<div class="monster-color-field">
  ...
  <div class="monster-color-controls">
    <input id="color-{label}" type="number" class="monster-color-number" min="0" max={outfitColorHexCache.length - 1} value={colorId} oninput={handleNumberInput} />
    <input type="color" class="monster-color-input" value={hexValue} oninput={handleColorInput} />
  </div>
</div>
