<script lang="ts">
  ...
  async function loadOutfitSprite(o: MonsterOutfit, preserveDirection = false) {
      stopAnimation();
      if (!preserveDirection) currentDirectionIndex = 0;

      try {
          const sprites = await getAppearanceSprites("Outfits", o.lookType);
          const newDirections = await getDirectionalSpritesForOutfit(o, sprites);
  ...
  async function getDirectionalSpritesForOutfit(o: MonsterOutfit, sprites: Uint8Array[]) {
      ...
      try {
          ...
          if (metadata) {
              ...
              const addonIndex = computeAddonPatternIndex(o.lookAddons, addonCount);
              const mountIndex = computeMountPatternIndex(o.lookMount, mountCount);
              ...
              for (let direction = 0; direction < directionCount; direction++) {
                  const renderResults = await Promise.all(
                      Array.from({ length: frameCount }, (_, phaseIndex) =>
                          composeOutfitSprite({
                              sprites, spriteInfo, baseOffset, direction, addonIndex, mountIndex, o, phaseIndex
                          })
                      )
                  );
                  ...
                  if (frames.length > 0) previews.push({ direction, frames, durations });
              }
  ...
  async function composeOutfitSprite(params: any): Promise<string | null> {
      const { sprites, spriteInfo, baseOffset, direction, addonIndex, mountIndex, o, phaseIndex } = params;
      ...
      return new Promise((resolve) => {
          ...
          outfitComposeWorker!.postMessage({
              id, layers, colors: { head: headColor, body: bodyColor, legs: legsColor, feet: feetColor }
          }, transferables);
      });
  }
...
<div class="outfit-preview">
    <div class="outfit-sprite-container">
        ...
        {/if}
        <button type="button" class="sprite-rotate-button" title="Rotate sprite" onclick={rotateSprite} disabled={previewDirections.length <= 1}>
            ⟳
        </button>
    </div>
    ...
    <div class="outfit-info">
        ...
        <div class="outfit-info-item"><div class="outfit-info-label">Body</div><div class="outfit-info-value">{outfit.lookBody}</div></div>
        <div class="outfit-info-item"><div class="outfit-info-label">Legs</div><div class="outfit-info-value">{outfit.lookLegs}</div></div>
