<script lang="ts">
  ...
  let normalizedFilter = $derived(monsterState.monsterSearchQuery.trim().toLowerCase());
  ...
  let sortedRootChildren = $derived(sortCategoryNodes(Array.from(tree.children.values()), monsterState.bestiaryClassOrder));
  let sortedRootMonsters = $derived([...tree.monsters].sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: 'base' })));
