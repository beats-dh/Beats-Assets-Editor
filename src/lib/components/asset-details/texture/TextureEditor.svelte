<script lang="ts">
  ...
  let textureCategory = $derived(
    assetsState.currentCategory === "Outfits" ||
      assetsState.currentCategory === "Objects"
      ? assetsState.currentCategory
      : "Other",
  );
  let isOutfit = $derived(textureCategory === "Outfits");
  let isUnsupported = $derived(textureCategory === "Other");
  let activeTextureCategory = $derived(
    isOutfit ? "Outfits" : textureCategory === "Objects" ? "Objects" : null,
  );
  let currentFrameGroup = $derived(
    details?.frame_groups
      ? details.frame_groups[viewState.frameGroupIndex]
      : undefined,
  );
  ...
  function getGroupOffset(): number {
    if (!details) return 0;
    return (
      computeGroupOffsetsFromDetails(details)[viewState.frameGroupIndex] ?? 0
    );
  }
  async function fetchSpriteBuffers(
    spriteIds: number[],
  ): Promise<Uint8Array[]> {
    return Promise.all(
      spriteIds.map(
        async (id) => (await getSpriteById(id)) ?? new Uint8Array(),
      ),
    );
  }
  ...
  function refreshDetailPreviews(): void {
    ...
    requestAnimationFrame(() =>
      loadDetailSprites(activeTextureCategory!, details.id, undefined, details),
    );
  }

  function handleStateChange(newState: any) {
    if (
      newState.frameGroupIndex !== undefined &&
      newState.frameGroupIndex !== viewState.frameGroupIndex
    ) {
      viewState = {
        ...viewState,
  ...
  async function handleReorder(detail: { newOrder: number[] }) {
    ...
    const reordered = detail.newOrder
      .map((i) => ids[i])
    ...
    try {
      await invoke("replace_appearance_sprites", {
        category,
        id: details.id,
        update: { frame_group_index: viewState.frameGroupIndex, updates },
      });
      ...
      const reorderedSprites = detail.newOrder.map(
        (index) => groupSprites[index] ?? new Uint8Array(),
      );
  ...
  async function handleRemove(detail: { indices: number[] }) {
    const category = activeTextureCategory;
    if (!category || !spriteInfo?.sprite_ids) return;
    const indices = Array.from(new Set(detail.indices))
    ...
    try {
      ...
      indices
        .slice()
        .sort((a, b) => b - a)
        .forEach((i) => spriteInfo!.sprite_ids?.splice(i, 1));
      if (details.frame_groups[viewState.frameGroupIndex])
        details.frame_groups[viewState.frameGroupIndex].sprite_info =
          spriteInfo;
      const offset = getGroupOffset();
      ...
      indices
        ...
        .forEach((i) => {
          const t = offset + i;
          if (t >= 0 && t < current.length) current.splice(t, 1);
        });
      sprites = current;
      removeCachedAppearanceSprites(
  ...
  async function handleReplace(detail: { index: number; spriteIds: number[] }) {
    ...
    spriteIds.forEach((spriteId, off) => {
      const ti = index + off;
      if (ti < spriteInfo!.sprite_ids.length)
        updates.push({ index: ti, sprite_id: spriteId });
      else appendIds.push(spriteId);
    });
    try {
      if (updates.length > 0) {
        await invoke("replace_appearance_sprites", {
        ...
        updates.forEach((u) => {
          if (spriteInfo!.sprite_ids && u.index < spriteInfo!.sprite_ids.length)
            spriteInfo!.sprite_ids[u.index] = u.sprite_id;
        });
      }
      ...
      if (updates.length > 0 || appendIds.length > 0) {
        ...
        const current = sprites.slice();
        if (updates.length > 0) {
          const buffers = await fetchSpriteBuffers(
            updates.map((u) => u.sprite_id),
          );
          updates.forEach((u, ui) => {
            const t = offset + u.index;
            if (t >= 0 && t < current.length)
              current[t] = buffers[ui] ?? new Uint8Array();
          });
        }
        if (appendIds.length > 0) {
  ...
  function handleSettingsChange(_detail: any) {
  ...
  function collectTextureUpdatePayload() {
    ...
    if (spriteInfo.animation) {
      const a = spriteInfo.animation;
      payload.animation = {
        loop_type: Number(a.loop_type || 0),
        loop_count: Number(a.loop_count || 0),
