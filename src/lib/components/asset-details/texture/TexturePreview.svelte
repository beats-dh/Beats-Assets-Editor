<script lang="ts">
  ...
  const textureDecodePending = new Map<
    string,
  ...
  function initTextureDecodeWorker() {
    ...
    try {
      ...
      textureDecodeWorker.onmessage = (
        event: MessageEvent<{ id: string; buffer: ArrayBuffer | null }>,
      ) => {
  ...
  function getFrameCount(info: CompleteSpriteInfo | undefined): number {
    if (!info) return 1;
    if (info.animation && info.animation.phases.length > 0)
      return info.animation.phases.length;
    return 1;
  }
  ...
  function drawBoundingSquareOverlay(
    ...
    ctx2d.strokeRect(0, 0, boundingSquare * scale, boundingSquare * scale);
    ctx2d.restore();
  }
  function getBoundingSquareValue(): number | null {
    if (!spriteInfo) return null;
    const v = Number(spriteInfo.bounding_square ?? "");
    return Number.isFinite(v) && v > 0 ? v : null;
  }
  function getPreviewBoundingBoxes(): Array<{
    x: number;
    y: number;
    width: number;
    height: number;
  }> {
    return (spriteInfo?.bounding_boxes ?? []).map((box) => ({
  ...
  function drawOutfitBoundingBoxes(
    boxes: Array<{ x: number; y: number; width: number; height: number }>,
    scale = 1,
  ) {
    if (!boxes.length || !ctx) return;
    const dirIdx = clamp(ps.direction, 0, boxes.length - 1);
    const box = boxes[dirIdx] || boxes[0];
    if (!box) return;
    ctx.save();
    ctx.strokeStyle = "#ff9800";
    ctx.lineWidth = 1;
    const x = ensureNumber(box.x, 0) * scale,
      y = ensureNumber(box.y, 0) * scale,
      w = ensureNumber(box.width, 0) * scale,
      h = ensureNumber(box.height, 0) * scale;
  ...
  async function loadImage(
    ...
    const promise = new Promise<HTMLImageElement>((resolve, reject) => {
      ...
      console.warn("Falling back to main thread decode", index, err);
      return new Promise<HTMLImageElement>((resolve, reject) => {
        const img = new Image();
        img.onload = () => resolve(img);
        img.onerror = (error) => reject(error);
        img.src = getSpriteUrl(sprite);
      });
    });
  ...
  async function renderSpriteVariant(
    ...
    const baseIndex =
      baseOffset +
      computeSpriteIndex(info, 0, direction, addon, mount, localFrame);
    ...
    if (ps.blendLayers && layers > 1) {
      const layerIndex =
        baseOffset +
        computeSpriteIndex(info, 1, direction, addon, mount, localFrame);
      const templateImage = await loadImage(layerIndex, sprites, imageCache);
      const tc = document.createElement("canvas");
      tc.width = templateImage.width;
      tc.height = templateImage.height;
      const tCtx = tc.getContext("2d");
      if (tCtx) {
        tCtx.drawImage(templateImage, 0, 0);
        const td = tCtx.getImageData(0, 0, tc.width, tc.height);
        const bd = offCtx.getImageData(0, 0, offscreen.width, offscreen.height);
        const head = hexToRgb(ps.headColor),
          body = hexToRgb(ps.bodyColor),
          legs = hexToRgb(ps.legsColor),
          feet = hexToRgb(ps.feetColor);
        for (let i = 0; i < td.data.length; i += 4) {
          ...
          if (!color) continue;
          bd.data[i] = Math.round((bd.data[i] + color.r) / 2);
          bd.data[i + 1] = Math.round((bd.data[i + 1] + color.g) / 2);
          bd.data[i + 2] = Math.round((bd.data[i + 2] + color.b) / 2);
        }
  ...
  async function drawOutfit(): Promise<void> {
    ...
    if (!info || !sprites || sprites.length === 0) {
    ...
    const groupOffs = computeGroupOffsetsFromDetails(details);
    const baseOffset = groupOffs[ps.frameGroupIndex] ?? 0;
    const dirMax = Math.max(0, ensureNumber(info.pattern_width, 1) - 1);
    const adMax = Math.max(0, ensureNumber(info.pattern_height, 1) - 1);
    const mtMax = Math.max(0, ensureNumber(info.pattern_depth, 1) - 1);
    const frameCount = getFrameCount(info);
    ps.direction = clamp(ps.direction, 0, dirMax);
    ps.addon = clamp(ps.addon, 0, adMax);
    ps.mount = clamp(ps.mount, 0, mtMax);
    ps.frame = clamp(ps.frame, 0, Math.max(frameCount - 1, 0));
    const variants: number[] = ps.showFullAddons
      ? Array.from({ length: adMax + 1 }, (_, i) => i)
      : [ps.addon];
    ...
    for (const addon of variants) {
      const rendered = await renderSpriteVariant(
        info,
        baseOffset,
        ps.direction,
        addon,
      ...
      if (!initialized) {
        const { width, height } = computePreviewDimensions(
          rendered.width,
          rendered.height,
          boxes,
          boundingSquare,
  ...
  async function drawObject(): Promise<void> {
    ...
    const groupOffs = computeGroupOffsetsFromDetails(details);
    const baseOffset = groupOffs[ps.frameGroupIndex] ?? 0;
    const pW = Math.max(1, ensureNumber(info.pattern_width, 1)),
      pH = Math.max(1, ensureNumber(info.pattern_height, 1)),
      pD = Math.max(1, ensureNumber(info.pattern_depth, 1)),
      layers = Math.max(1, ensureNumber(info.layers, 1));
    const frameCount = getFrameCount(info);
    ps.patternX = clamp(ps.patternX, 0, pW - 1);
    ps.patternY = clamp(ps.patternY, 0, pH - 1);
    ps.patternZ = clamp(ps.patternZ, 0, pD - 1);
    ps.layer = clamp(ps.layer, 0, layers - 1);
    ps.frame = clamp(ps.frame, 0, Math.max(frameCount - 1, 0));
    const aggIdx =
      baseOffset +
      computeSpriteIndex(
    ...
    if (ps.showBoundingBoxes) {
      drawBoundingSquareOverlay(ctx, boundingSquare, scale);
      if (boxes.length > 0) {
        const di = clamp(ps.patternX, 0, boxes.length - 1);
        const box = boxes[di] || boxes[0];
        if (box) {
          ctx.save();
  ...
  async function draw() {
    if (isOutfit) await drawOutfit();
    else await drawObject();
  }
  ...
  function startAnimation() {
    ...
    if (frameCount > 1) {
      const dur = Math.max(
        50,
        spriteInfo?.animation?.phases?.[ps.frame]?.duration_min ?? 250,
      );
      animationTimer = window.setInterval(() => {
        ps.frame = (ps.frame + 1) % frameCount;
        onStateChange?.({ frame: ps.frame });
        draw();
      }, dur);
    }
  }
  ...
  onDestroy(() => {
    stopAnimation();
    if (textureDecodeWorker) textureDecodeWorker.terminate();
    textureDecodePending.clear();
  });
  ...
  $effect(() => {
    if (ps?.autoAnimate && isOutfit) startAnimation();
    else stopAnimation();
  });
...
>
  <div class="texture-drop-hint">
    ...
    <div class="texture-drop-subtitle">
      {translate("texture.drop.subtitle")}
    </div>
  </div>
  <canvas
    bind:this={canvas}
