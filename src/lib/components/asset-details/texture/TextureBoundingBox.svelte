<script lang="ts">
  ...
  function removeBoundingBox(index: number) {
    ...
    spriteInfo.bounding_boxes = spriteInfo.bounding_boxes.filter(
      (_, i) => i !== index,
    );
    onChange?.({ bounding_boxes: spriteInfo.bounding_boxes });
  }
  ...
  <div class="texture-form-section texture-bounding-section">
    ...
    <div class="texture-bounding-boxes">
      <table class="texture-bounding-table">
        <thead
          ><tr
            ><th>{translate("texture.bounding.header.index")}</th><th
              >{translate("texture.bounding.header.x")}</th
            ><th>{translate("texture.bounding.header.y")}</th><th
              >{translate("texture.bounding.header.width")}</th
            ><th>{translate("texture.bounding.header.height")}</th><th
              >{translate("texture.bounding.header.actions")}</th
            ></tr
          ></thead
        >
        <tbody id="texture-bounding-box-body">
          {#if !spriteInfo.bounding_boxes || spriteInfo.bounding_boxes.length === 0}
            <tr class="texture-empty-row"
              ><td colspan="6">{translate("texture.bounding.empty")}</td></tr
            >
          {:else}
            {#each spriteInfo.bounding_boxes as box, i}
              <tr>
                ...
                <td
                  ><button
                    type="button"
                    class="texture-remove-box"
                    onclick={() => removeBoundingBox(i)}>&times;</button
                  ></td
                >
              </tr>
            {/each}
      ...
      <button
        id="texture-add-bounding-box"
        class="btn-secondary"
        onclick={addBoundingBox}
        >{translate("texture.bounding.button.add")}</button
      >
    </div>
  </div>
{/if}
