<script lang="ts">
  ...
  interface Props { spriteInfo: CompleteSpriteInfo | undefined; onChange?: (detail: any) => void; onSave?: () => void; }
...
<div class="texture-form-section">
  ...
  <div class="texture-form-grid">
    <label><span>{translate('texture.form.patternWidth')}</span><input type="number" id="texture-pattern-width" min="0" bind:value={spriteInfo.pattern_width} /></label>
...
<div class="texture-form-section">
  ...
  {#if spriteInfo.animation?.phases}
    <div id="texture-animation-phases" class="texture-animation-phases">
      {#if spriteInfo.animation.phases.length === 0}<p class="texture-empty">{translate('texture.animation.empty')}</p>
      {:else}{#each spriteInfo.animation.phases as phase, i}
        <div class="texture-phase-row">
          <label><span class="texture-phase-label">{translate('texture.animation.phaseMin', { index: i + 1 })}</span><input type="number" class="texture-phase-min" min="0" bind:value={phase.duration_min} /></label>
          <label><span class="texture-phase-label">{translate('texture.animation.phaseMax')}</span><input type="number" class="texture-phase-max" min="0" bind:value={phase.duration_max} /></label>
        </div>
      {/each}{/if}
    </div>
  {/if}
</div>
<div class="texture-form-actions"><button id="texture-save-button" class="btn-primary" onclick={() => onSave?.()}>{translate('texture.button.save')}</button></div>
{/if}
