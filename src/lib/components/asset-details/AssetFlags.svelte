<script lang="ts">
  ...
  interface Props { flags: CompleteFlags; }
...
{#if flags}
  ...
  {#if (flags.hook?.direction !== undefined) || (flags.hook_south !== undefined) || (flags.hook_east !== undefined)}
    <div class="detail-section"><h4>Hook Properties</h4>
      {#if flags.hook?.direction !== undefined}<div class="detail-item"><span class="detail-label">Hook Direction:</span><span class="detail-value">{getHookDirectionName(flags.hook.direction)}</span></div>{/if}
      {#if flags.hook_south !== undefined}<div class="detail-item"><span class="detail-label">Hook South:</span><span class="detail-value">{flags.hook_south ? '✅ Yes' : '❌ No'}</span></div>{/if}
      {#if flags.hook_east !== undefined}<div class="detail-item"><span class="detail-label">Hook East:</span><span class="detail-value">{flags.hook_east ? '✅ Yes' : '❌ No'}</span></div>{/if}
    </div>
  {/if}
  ...
  {#if flags.market}
    <div class="detail-section"><h4>Market Information</h4>
      ...
      {#if flags.market.name}<div class="detail-item"><span class="detail-label">Market Name:</span><span class="detail-value">{flags.market.name}</span></div>{/if}
      {#if flags.market.minimum_level !== undefined}<div class="detail-item"><span class="detail-label">Minimum Level:</span><span class="detail-value">{flags.market.minimum_level}</span></div>{/if}
      {#if flags.market.vocation !== undefined}<div class="detail-item"><span class="detail-label">Vocation:</span><span class="detail-value">{getVocationName(flags.market.vocation)}</span></div>{/if}
      ...
      {#if flags.market.restrict_to_vocation && flags.market.restrict_to_vocation.length > 0}<div class="detail-item"><span class="detail-label">Restrict To Vocation:</span><span class="detail-value">{flags.market.restrict_to_vocation.map(v => getVocationName(v)).join(', ')}</span></div>{/if}
    </div>
  {/if}
